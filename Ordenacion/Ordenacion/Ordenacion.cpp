...
namespace algoritmosDeOrdenacion {
   ...
   ///Elegimos un numero al alazar (Mitad de la cadena)
    ...
    ///Obtenemos valores random y creamos un std::vector
    template <class T>
    vector<T> getValuesRandom(size_t size) {
        vector<T> list;
        for (size_t i = 0; i < size; i++) {
...
using namespace algoritmosDeOrdenacion;

int main()
{
    srand(time(NULL));
    const int tamaño = 10;
    auto toOrder = getValuesRandom<short>(tamaño);
    quickSort<short>(toOrder.data(), tamaño);

    for (unsigned int i = 0; i < tamaño; i++)
    {
        cout << toOrder[i] << " " << ((i + 1 == tamaño )? "" : "- ");
    }
}
